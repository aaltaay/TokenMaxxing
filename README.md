...
- **Pace:** for every window with a reported reset (Claude 5-hour and weekly, Codex windows, the Cursor billing cycle), the app compares usage with the even rate that would reach 100% exactly at the reset. It shows a tick for even pace on each bar, how many times faster than that you are spending, when the quota runs out at the current rate, and the rate that would last. Windows running ahead of pace surface as an Overview alert. This is a straight line through the provider's own numbers, not a prediction of your workload.
...
The status line writes only a session id, model name, token counters, and reported cost to `~/.tokenmaxxing/claude-active.json` (override the folder with `TOKENMAXXING_HOME`). No prompt text, transcript content, file path, or keystroke is read or stored. Without it (the Claude desktop app does not run status lines), **Open Claude Code session** reads the open session's title from the Claude desktop window on Windows, the same way the open Codex chat is read, and matches it to the live-session registry Claude Code keeps under `~/.claude/sessions/`. Switching chats in the desktop app changes the displayed session within a few seconds. When no window title is available, the Claude Code log written to in the last 15 minutes is followed and labelled as such; older logs are never shown as open. Claude sessions are listed under the titles the desktop app shows. **Latest activity** and manual selection also work.
